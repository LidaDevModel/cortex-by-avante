...
function SidebarProvider({
  ...
  // Drawer mode below the nav breakpoint (1024), not 768 — see use-nav-shape.
  // The context field keeps its name: to the sidebar system it has always
  // meant "am I an off-canvas drawer right now".
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon-sm"
      // Sidebar-only: the trigger toggles the rail where a rail exists. Below
      // the nav breakpoint the sidebar is an off-canvas drawer, opened by the
      // header burger (admins) — learners get the floating pill instead.
...
function SidebarInset({ className, style, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex w-full flex-1 flex-col lg:peer-data-[variant=inset]:m-1 lg:peer-data-[variant=inset]:ml-0 lg:peer-data-[variant=inset]:rounded-xl lg:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-1",
        className
      )}
      style={{
...
const sidebarMenuButtonVariants = cva(
  // [data-mobile=true] is the off-canvas drawer (tablet + phone) — the only
  // touch-driven sidebar, so rows there meet the 44px hit-target floor.
  // Pointer widths keep the compact 32px row.
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left type-nav [[data-mobile=true]_&]:min-h-11 [[data-mobile=true]_&]:px-3 ring-sidebar-ring outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground [[data-mobile=true]_&]:min-h-11 [[data-mobile=true]_&]:px-3 ring-sidebar-ring outline-hidden group-data-[collapsible=icon]:hidden hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[size=md]:text-sm data-[size=sm]:text-xs data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground",
        className
      )}
      {...props}
    />
