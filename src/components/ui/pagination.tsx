...
export function Pagination({ page, totalPages, onChange, className }: PaginationProps) {
  ...
  const navBtn = "size-11 md:size-8 flex items-center justify-center rounded-[6px] border border-border text-[13px] leading-[20px] transition-colors duration-100 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-[var(--surface-raised)]";
        ...
        {pages.map((p, i) =>
          p === "…" ? (
            ...
            <button
              key={p}
              onClick={() => onChange(p)}
              className="size-11 md:size-8 flex items-center justify-center rounded-[6px] border text-[13px] leading-[20px] font-medium transition-colors duration-100"
              style={
                p === page
